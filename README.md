...
| **6** | **Run Evaluation Pipeline** | Navigate to: `Features/evaluation/`<br>Run in order:<br>1. `word2vec_evaluation.ipynb` — BiLSTM Word2Vec embedding evaluation<br>2. `bilstm_evaluation_cdtesting.ipnyb` — BiLSTM base + ablation model performance evaluation and cross-domain testing<br>3. `distilBERT_evaluation_cdtesting.ipynb` — DistilBERT base + ablation model performance evaluation and cross-domain testing |
